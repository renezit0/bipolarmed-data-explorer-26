...
interface UseMultiStateMedicDataProps {
  tables: string[];
  labels?: string[];
}
...
export const useMultiStateMedicData = ({ tables, labels }: UseMultiStateMedicDataProps) => {
      ...
      try {
        ...
        const promises = tables.map(async (tableName) => {
          ...
          return rawData.map((row: any) => {
            ...
            return {
              procedimento: fullName.split(' ')[0] || '',
              simplifiedName,
              fullName,
              timeSeriesData,
              totalConsumption
            };
          }).filter((item: ProcessedMedicData) => item.totalConsumption > 0);
        });
  ...
  return { data, loading, error, labels: labels || tables };
};
