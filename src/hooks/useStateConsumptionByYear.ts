...
import { extractYearFromPeriod } from '@/constants/historicalPopulation';
...
export interface StateConsumptionByYear {
  [stateTable: string]: {
    [year: number]: number;
  };
}

export const useStateConsumptionByYear = () => {
  const [consumptionByStateYear, setConsumptionByStateYear] = useState<StateConsumptionByYear>({});
  ...
  useEffect(() => {
    const fetchAllStates = async () => {
      try {
        ...
        const stateConsumption: StateConsumptionByYear = {};
        ...
        const allPromises = Object.entries(STATES).map(async ([code, stateInfo]) => {
          ...
          if (!rawData || rawData.length === 0) {
            stateConsumption[stateInfo.table] = {};
            return;
          }
          ...
          stateConsumption[stateInfo.table] = consumptionByYear;
        });

        await Promise.all(allPromises);

        console.log('✅ Consumo por estado e ano calculado:', stateConsumption);
        setConsumptionByStateYear(stateConsumption);
      } catch (err) {
        console.error('💥 Erro ao calcular consumo por estado e ano:', err);
  ...
  return { consumptionByStateYear, loading, error };
};
